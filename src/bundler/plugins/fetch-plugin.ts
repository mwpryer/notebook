  ...
  return {
    ...
    setup(build: esbuild.PluginBuild) {
      ...
      build.onLoad({ filter: /(\.css$)/ }, async (args: esbuild.OnLoadArgs) => {
        ...
        `
        const result: esbuild.OnLoadResult = { contents, resolveDir: new URL("./", request.responseURL).pathname, loader: "jsx" }
      ...
      build.onLoad({ filter: /.*/ }, async (args: esbuild.OnLoadArgs) => {
        const { data, request } = await axios.get(args.path)
        const result: esbuild.OnLoadResult = { contents: data, resolveDir: new URL("./", request.responseURL).pathname, loader: "jsx" }
        await cache.setItem(args.path, result)
        return result
      })
    },
